...
- **No verificado de punta a punta con Google/Facebook real**: `tenant_identity_provider` sigue vacío para este tenant (nadie ha cargado credenciales reales) — el mecanismo de redirect está desplegado y activado, pero el botón de login social en sí no tiene con qué autenticar todavía. Es la misma pieza operativa (no código) ya anotada en el blueprint 08.2.
- **PROD**: pendiente del mismo gate manual de Marco que ticket 054 — sin cambios de código adicionales una vez aprobado.
