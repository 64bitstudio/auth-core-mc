...
- **Mejora continua propuesta, no implementada (fuera de alcance):** el servidor de dev cachea templates de Thymeleaf en memoria y `bootRun` no recopia recursos estáticos en caliente — forzó reinicios manuales para iterar. Sugerido `spring-boot-devtools` + `spring.thymeleaf.cache=false` en el perfil de dev como ticket de tooling aparte.
