...
# (eso todavia esta pendiente del DNS), asi que se puede correr ya mismo.
#
# El agente (Claude Code) quedo bloqueado por el clasificador de permisos
# al intentar correr esto el mismo -- pensado para que Marco lo corra
# directamente. Un solo comando:
#
...
# tocar la BD si el hash no coincide.
...
echo "== 1/4: desenvolviendo la data-key de Acme con el Vault local =="
...
echo "== 2/4: envolviendo la MISMA data-key con la Vault de la VM (platform-admin, via SSH) =="
...
echo "== 3/4: verificando el round-trip (debe dar la MISMA data-key cruda) =="
...
echo "== 4/4: actualizando tenant.wrapped_data_key en la base de datos local =="
docker exec "$DB_CONTAINER" psql -U "$DB_USER" -d "$DB_NAME" -c \
  "UPDATE tenant SET wrapped_data_key = '$NEW_WRAPPED' WHERE id = '$TENANT_ID';"

echo ""
echo "Listo. El tenant Acme (Google/Facebook) ya usa la Vault de la VM -- verificar con una operacion real de la app cuando application.properties apunte ahi."
