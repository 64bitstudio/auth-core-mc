...
| DELETE | `/api/v1/account/connected-providers/{provider}` | Header `Authorization: Bearer <accessToken>`; `{provider}` = `google` o `facebook` | `204` (desvinculado), `404 provider_not_linked` (nunca estuvo vinculado a este usuario), `409 cannot_unlink_last_login_method` (el usuario no tiene contraseña y este es su único proveedor — nunca lo deja sin forma de entrar), o `400 unsupported_provider` |
...
| Método | Ruta | Qué hace |
|---|---|---|
| GET | `/api/v1/oauth2/login-url/{provider}` | **Ticket `072`** — resuelve la URL de abajo (`/oauth2/authorization/{identityClientId}::{provider}`) para un visitante SIN sesión, que no puede conocer el UUID interno del `IdentityClient`. Header `X-Client-Id`, SIN `Authorization` (público a propósito, mismo criterio que las 3 rutas de abajo). `200` + `{redirectUrl}` (URL absoluta, mismo formato que `POST /account/link-provider/{provider}` para un usuario ya autenticado — ver ese endpoint más abajo), o `400 unsupported_provider`, o `401 unknown_client` |
