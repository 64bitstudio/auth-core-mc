...
## Tickets 065-068: la Pantalla "Usuario" de galgoth-studio expone tres huecos "solo probados same-origin"

**Contexto**: galgoth-studio#093 ("Pantalla Usuario") es el primer caller REAL, en navegador, cross-origin (`hosts_own_login_ui = true`, ticket 056) de tres mecanismos de este backend que hasta ahora solo se habían probado desde la propia UI hospedada de auth-core-mc (mismo origen) o vía MockMvc directo (sin CORS, sin Traefik, sin navegador de por medio). Los tres hoyos comparten el mismo patrón de fondo: código correcto para el único caller que existía en su momento, expuesto por el primer caller nuevo.

- **Ticket 065** (trivial, sin decisión): `UserResponse` nunca exponía `createdAt` — "Miembro desde {fecha}" de la Pantalla Usuario necesitaba un campo que ya existía en `User` desde el ticket 001, solo faltaba exponerlo.
- **Ticket 066**: `CorsConfig.allowedHeaders` (ticket 054) nunca incluyó `X-Current-Refresh-Token` (ticket 062) — nadie lo había mandado antes desde un navegador cruzando origen. Preflight real confirmado antes del fix (`Access-Control-Allow-Headers: authorization, content-type`, sin el tercero) y después (con él).
- **Ticket 067**: `AccountLinkProviderController.linkProvider` (ticket 063) devolvía una ruta relativa — resuelta contra el origen del CALLER (galgoth-studio), no el de auth-core-mc, por un `window.location.href` cross-origin. Y `LinkIntentSession` (misma cookie de sesión que el resto del proyecto ya usa para correlacionar el callback de OAuth2) nunca sobrevivía el viaje de ida y vuelta porque el navegador descarta cualquier `Set-Cookie` de una respuesta cross-origin sin `credentials: 'include'`/`Access-Control-Allow-Credentials`. **Decisión con VoBo explícito de Marco** (cambio de seguridad real, no "de paso"): reutilizar el mecanismo de sesión existente vía cookie cross-site (`SameSite=None` + `Secure`, solo en el perfil `deploy` — nunca local/tests, donde `http://localhost:8080` sigue siendo un `redirect_uri` real registrado para pruebas de OAuth2) en vez de rediseñar la correlación con un token nuevo.
- **Ticket 068**: primera prueba real en navegador de CUALQUIER flujo `oauth2Login` de este proyecto, detrás de Traefik — expuso que Spring construía el `redirect_uri` de OAuth2 con el esquema de la conexión INTERNA (`http://`, Traefik reenvía puertas adentro por HTTP plano) en vez del externo real (`https://`), y Google lo rechazaba con `redirect_uri_mismatch`. Fix de una línea (`server.forward-headers-strategy=framework`, solo en `deploy`) — pero **incompleto sin `platform#008`** (Traefik necesitó su propio fix de `forwardedHeaders.trustedIPs` para confiar en el `X-Forwarded-Proto` real que nginx ya mandaba correctamente; ver `platform/docs/ARQUITECTURA.md`).

Los cuatro se verificaron en vivo contra dev como una sola sesión de QA: cuenta de prueba real registrada, recorrido completo de la Pantalla Usuario (editar info personal, cambiar contraseña, revisar sesiones, vincular una cuenta de Google real de punta a punta incluyendo el consentimiento real, alternar preferencias, cerrar sesión en todos los dispositivos, eliminar la cuenta al final) — no simulado.

## Ticket 071: se corrige el "límite de confianza temporal" que sí resultó explotable

Los tickets 003/005 (`/verify-email`, `/change-email`, `/2fa`) y las notas de arquitectura de más arriba (líneas del "límite de confianza temporal", `TenantScopedUserResolver`) documentaban un `userId` confiado del body como una decisión consciente y acotada — "molesto, no explotable" porque completar cualquiera de esos flujos exigía poseer un correo/SMS que la víctima controla. Esa suposición nunca se re-validó tras el ticket 007 (Authorization Server real, que sí migró el resto de `/account/**`) ni tras el hallazgo del ticket 084/091 de galgoth-studio (`ProjectSummary.avatarUrl` expone el UUID real del dueño en un endpoint público, `GET /api/explore/projects`).

**Por qué la suposición original era falsa en 2 de los 3 casos:**
- `change-email/request` manda su correo de confirmación a la dirección NUEVA que el propio atacante elige — el atacante posee ese "segundo factor" trivialmente. Encadenado con el leak de UUID de galgoth-studio: filtrar el `userId` de una víctima real desde Explorar → pedir el cambio → confirmarlo con el link que le llega a su propio correo (la víctima real nunca recibe ningún aviso) → reset de contraseña con el correo nuevo → cuenta secuestrada de punta a punta.
- `2fa/totp/enroll` no manda nada a ningún canal de la víctima — devuelve el secreto TOTP directo en la respuesta. Un atacante podía enrolar su propio secreto en la cuenta de otra persona y activarlo con `/2fa/method`, bloqueándola afuera permanentemente sin su consentimiento.
- `verify-email/request` sí sostiene razonablemente la suposición original (solo reenvía a la dirección ya registrada, cooldown de 60s) — **se queda tal cual, a propósito, no por descuido**. Migrarlo además habría roto el único caller real que lo usa sin sesión: galgoth-studio dispara el primer correo de verificación justo después de `/api/v1/register`, que deliberadamente no entrega tokens (`PROP-GS-AUTH-01` Fig. 02) — no hay ningún JWT disponible en ese momento para exigir.

**Corrección, 2 de los 3** (ver `docs/API.md` y el Javadoc de `EmailChangeController`/`TwoFactorController`): esos dos controllers sacan el usuario exclusivamente del claim `sub` del JWT verificado, igual que `AccountProfileController`/`SetPasswordController` ya hacían — nunca de un `userId` en el body. `/api/v1/2fa/**` sale por completo de `permitAll` en `SecurityConfig`; `/change-email/**` se acota a solo su ruta `/confirm` (la única mitad que de verdad es pública por diseño). `/api/v1/verify-email/**` se queda completo en `permitAll`, sin cambio. `TenantScopedUserResolver` sobrevive con un solo caller restante (`EmailVerificationController`) y su Javadoc se actualiza para reflejarlo — ya no es "temporal", es la decisión final para ese endpoint puntual. `cuenta.html` (la UI hospedada que de verdad ejercita estos 6 endpoints migrados, más el 1 que no) migra esas 6 llamadas de `AuthCoreUi.call` a `AuthCoreUi.callAuthenticated`, mismo patrón que "Establecer contraseña" desde el ticket 041 — el reenvío de verificación se queda como estaba.

Detalle completo, criterios de aceptación y tests en el ticket 071 (`071-fix-trust-boundary-userid-2fa-emails.md`).
