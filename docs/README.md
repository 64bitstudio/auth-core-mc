...
3. Entra a `backend/` y corre `./gradlew bootRun` — Spring Boot levanta automáticamente Postgres y Redis vía `backend/compose.yaml` (soporte nativo de Docker Compose de Spring Boot), corre las migraciones Flyway, y arranca la API en `http://localhost:8080`.
   - **Hot-reload de templates/estáticos (ticket `033`)**: con `bootRun` corriendo, `spring-boot-devtools` (dependencia `developmentOnly`, nunca llega a producción ni a los tests) deshabilita automáticamente el caché de Thymeleaf y de recursos estáticos. Un cambio en `templates/**` o `static/**` se refleja al recargar el navegador sin reiniciar manualmente el proceso — solo hace falta que el archivo cambiado se recopie a `build/resources/main`. Si editas los archivos directamente en un editor de texto (fuera de una IDE con auto-build), corre `./gradlew processResources` (o `./gradlew -t processResources` en otra terminal para que se repita solo) después de cada cambio para disparar la recopia + el restart automático de devtools.
