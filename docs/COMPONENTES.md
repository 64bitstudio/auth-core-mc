...
### Paso intermedio de 2FA (`fragments/two-factor-step.html`, ticket `046`)
Cierra el gap de UI que el ticket `045` dejó explícito: cuando `POST /api/v1/login` (password) o `POST /api/v1/oauth2/social-exchange` (social) responde `202 twoFactorRequired` (ticket `045`), `login.html`/`social-callback.html` ocultan su propio formulario/spinner y muestran este mismo fragmento compartido en su lugar — un solo componente para ambos flujos de entrada, cableado por `AuthCoreUi.startTwoFactorStep(pendingToken, method, statusEl, onVerified)` (`static/js/api.js`), no dos implementaciones paralelas.

- **Markup server-side, comportamiento en JS** — mismo criterio que el resto de este proyecto (ver el comentario de `icons.html` sobre por qué el SVG nunca se construye como string de JS): el fragmento ya trae el formulario de código, el botón "Reenviar código" (oculto por defecto) y el mensaje de expiración; `startTwoFactorStep` solo los revela/cablea, no construye DOM.
- **Reenvío condicionado al método**: el botón "Reenviar código" solo se muestra para `OTP_EMAIL`/`OTP_SMS` (llama a `POST /api/v1/login/2fa-resend`, ver `API.md`) — nunca para `TOTP`, cuyo código ya vive en la app autenticadora del usuario.
- **Expiración proactiva del lado del cliente**: un `setTimeout` de 5 minutos (mismo TTL que `LoginCompletionService.PENDING_2FA_TTL`, documentado como constante en `api.js` para no desincronizarse en silencio) deshabilita el formulario y muestra "El código expiró. Vuelve a iniciar sesión." **antes** de que un submit falle contra un `pendingToken` ya vencido — puramente una mejora de UX, la expiración real la sigue imponiendo el TTL de Redis del lado del servidor.
- **Sin cambio de contrato de API** — ambos endpoints nuevos/existentes (`2fa-verify`, `2fa-resend`) se consumen tal cual, sin DTO nuevo del lado del cliente.
